/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

export const getAuditLogQueryParamsActionEnum = {
    "clan_application.create": "clan_application.create",
    "clan_application.accepted": "clan_application.accepted",
    "clan_application.rejected": "clan_application.rejected",
    "clan_application.pending": "clan_application.pending",
    "clan_application.delete": "clan_application.delete",
    "clan_application.clear_accepted": "clan_application.clear_accepted",
    "cwl_application.create": "cwl_application.create",
    "cwl_application.assign": "cwl_application.assign",
    "cwl_application.unassign": "cwl_application.unassign",
    "cwl_application.bulk_assign": "cwl_application.bulk_assign",
    "cwl_application.bulk_unassign": "cwl_application.bulk_unassign",
    "cwl_application.bulk_delete": "cwl_application.bulk_delete",
    "cwl_application.update_notes": "cwl_application.update_notes",
    "cwl_bonus.tick": "cwl_bonus.tick",
    "cwl_bonus.untick": "cwl_bonus.untick",
    "cwl_season.create": "cwl_season.create",
    "cwl_season.rename": "cwl_season.rename",
    "cwl_season.delete": "cwl_season.delete",
    "settings.update": "settings.update",
    "cwl_ping.settings_update": "cwl_ping.settings_update",
    "cwl_ping.manual_run": "cwl_ping.manual_run",
    "rules.update": "rules.update",
    "privacy.update": "privacy.update",
    "clan.create": "clan.create",
    "clan.update": "clan.update",
    "clan.delete": "clan.delete",
    "cwl_clan.create": "cwl_clan.create",
    "cwl_clan.update": "cwl_clan.update",
    "cwl_clan.delete": "cwl_clan.delete",
    "cwl_clan.sync_leagues": "cwl_clan.sync_leagues",
    "coc_account.create": "coc_account.create",
    "coc_account.import": "coc_account.import",
    "coc_account.stats_update": "coc_account.stats_update",
    "coc_account.mark_main": "coc_account.mark_main",
    "coc_account.mark_external": "coc_account.mark_external",
    "coc_account.sync": "coc_account.sync",
    "coc_account.sync_war_weights": "coc_account.sync_war_weights",
    "coc_account.delete": "coc_account.delete",
    "coc_account.bulk_delete": "coc_account.bulk_delete",
    "user.role_set": "user.role_set",
    "user.create": "user.create",
    "user.update": "user.update",
    "user.ban": "user.ban",
    "user.unban": "user.unban",
    "user.remove": "user.remove",
    "user.password_set": "user.password_set",
    "user.session_revoked": "user.session_revoked",
    "user.sessions_revoked": "user.sessions_revoked",
    "user.api_access_granted": "user.api_access_granted",
    "user.api_access_revoked": "user.api_access_revoked",
    "api_key.create": "api_key.create",
    "api_key.update": "api_key.update",
    "api_key.delete": "api_key.delete",
} as const;

export type GetAuditLogQueryParamsActionEnumKey = (typeof getAuditLogQueryParamsActionEnum)[keyof typeof getAuditLogQueryParamsActionEnum];

export const getAuditLogQueryParamsTargetTypeEnum = {
    clan_application: "clan_application",
    cwl_application: "cwl_application",
    cwl_bonus: "cwl_bonus",
    cwl_season: "cwl_season",
    settings: "settings",
    rules: "rules",
    privacy: "privacy",
    clan: "clan",
    cwl_clan: "cwl_clan",
    coc_account: "coc_account",
    user: "user",
    api_key: "api_key",
} as const;

export type GetAuditLogQueryParamsTargetTypeEnumKey =
    (typeof getAuditLogQueryParamsTargetTypeEnum)[keyof typeof getAuditLogQueryParamsTargetTypeEnum];

export const getAuditLogQueryParamsSourceEnum = {
    web: "web",
    api: "api",
} as const;

export type GetAuditLogQueryParamsSourceEnumKey = (typeof getAuditLogQueryParamsSourceEnum)[keyof typeof getAuditLogQueryParamsSourceEnum];

export type GetAuditLogQueryParams = {
    /**
     * @type string | undefined
     */
    actorId?: string;
    /**
     * @type string | undefined
     */
    action?: GetAuditLogQueryParamsActionEnumKey;
    /**
     * @type string | undefined
     */
    targetType?: GetAuditLogQueryParamsTargetTypeEnumKey;
    /**
     * @type string | undefined
     */
    targetId?: string;
    /**
     * @type string | undefined
     */
    source?: GetAuditLogQueryParamsSourceEnumKey;
    /**
     * @type string | undefined, date-time
     */
    before?: string;
    /**
     * @type string | undefined, date-time
     */
    after?: string;
    /**
     * @minLength 1
     * @maxLength 200
     * @default 50
     * @type integer | undefined
     */
    limit?: number;
    /**
     * @minLength 0
     * @maxLength 9007199254740991
     * @default 0
     * @type integer | undefined
     */
    offset?: number;
};

export const entriesActorCurrentRoleEnum = {
    unverified: "unverified",
    verified: "verified",
    reviewer: "reviewer",
    manager: "manager",
    admin: "admin",
    superadmin: "superadmin",
} as const;

export type EntriesActorCurrentRoleEnumKey = (typeof entriesActorCurrentRoleEnum)[keyof typeof entriesActorCurrentRoleEnum];

export const entriesActionEnum = {
    "clan_application.create": "clan_application.create",
    "clan_application.accepted": "clan_application.accepted",
    "clan_application.rejected": "clan_application.rejected",
    "clan_application.pending": "clan_application.pending",
    "clan_application.delete": "clan_application.delete",
    "clan_application.clear_accepted": "clan_application.clear_accepted",
    "cwl_application.create": "cwl_application.create",
    "cwl_application.assign": "cwl_application.assign",
    "cwl_application.unassign": "cwl_application.unassign",
    "cwl_application.bulk_assign": "cwl_application.bulk_assign",
    "cwl_application.bulk_unassign": "cwl_application.bulk_unassign",
    "cwl_application.bulk_delete": "cwl_application.bulk_delete",
    "cwl_application.update_notes": "cwl_application.update_notes",
    "cwl_bonus.tick": "cwl_bonus.tick",
    "cwl_bonus.untick": "cwl_bonus.untick",
    "cwl_season.create": "cwl_season.create",
    "cwl_season.rename": "cwl_season.rename",
    "cwl_season.delete": "cwl_season.delete",
    "settings.update": "settings.update",
    "cwl_ping.settings_update": "cwl_ping.settings_update",
    "cwl_ping.manual_run": "cwl_ping.manual_run",
    "rules.update": "rules.update",
    "privacy.update": "privacy.update",
    "clan.create": "clan.create",
    "clan.update": "clan.update",
    "clan.delete": "clan.delete",
    "cwl_clan.create": "cwl_clan.create",
    "cwl_clan.update": "cwl_clan.update",
    "cwl_clan.delete": "cwl_clan.delete",
    "cwl_clan.sync_leagues": "cwl_clan.sync_leagues",
    "coc_account.create": "coc_account.create",
    "coc_account.import": "coc_account.import",
    "coc_account.stats_update": "coc_account.stats_update",
    "coc_account.mark_main": "coc_account.mark_main",
    "coc_account.mark_external": "coc_account.mark_external",
    "coc_account.sync": "coc_account.sync",
    "coc_account.sync_war_weights": "coc_account.sync_war_weights",
    "coc_account.delete": "coc_account.delete",
    "coc_account.bulk_delete": "coc_account.bulk_delete",
    "user.role_set": "user.role_set",
    "user.create": "user.create",
    "user.update": "user.update",
    "user.ban": "user.ban",
    "user.unban": "user.unban",
    "user.remove": "user.remove",
    "user.password_set": "user.password_set",
    "user.session_revoked": "user.session_revoked",
    "user.sessions_revoked": "user.sessions_revoked",
    "user.api_access_granted": "user.api_access_granted",
    "user.api_access_revoked": "user.api_access_revoked",
    "api_key.create": "api_key.create",
    "api_key.update": "api_key.update",
    "api_key.delete": "api_key.delete",
} as const;

export type EntriesActionEnumKey = (typeof entriesActionEnum)[keyof typeof entriesActionEnum];

export const entriesTargetTypeEnum = {
    clan_application: "clan_application",
    cwl_application: "cwl_application",
    cwl_bonus: "cwl_bonus",
    cwl_season: "cwl_season",
    settings: "settings",
    rules: "rules",
    privacy: "privacy",
    clan: "clan",
    cwl_clan: "cwl_clan",
    coc_account: "coc_account",
    user: "user",
    api_key: "api_key",
} as const;

export type EntriesTargetTypeEnumKey = (typeof entriesTargetTypeEnum)[keyof typeof entriesTargetTypeEnum];

export const entriesSourceEnum = {
    web: "web",
    api: "api",
} as const;

export type EntriesSourceEnumKey = (typeof entriesSourceEnum)[keyof typeof entriesSourceEnum];

/**
 * @description Audit log entries.
 */
export type GetAuditLog200 = {
    /**
     * @type boolean
     */
    success: true;
    /**
     * @type object
     */
    data: {
        /**
         * @type array
         */
        entries: {
            /**
             * @type number
             */
            id: number;
            actorId: string | null;
            actorName: string | null;
            actorCurrentName: string | null;
            actorCurrentImage: string | null;
            actorCurrentRole: EntriesActorCurrentRoleEnumKey | null;
            actorDiscordId: string | null;
            /**
             * @type string
             */
            action: EntriesActionEnumKey;
            targetType: EntriesTargetTypeEnumKey | null;
            targetId: string | null;
            metadata: any | null;
            /**
             * @type string
             */
            source: EntriesSourceEnumKey;
            apiKeyId: string | null;
            apiKeyName: string | null;
            /**
             * @type string, date-time
             */
            createdAt: string;
        }[];
        /**
         * @type number
         */
        total: number;
    };
};

/**
 * @description Unauthorized.
 */
export type GetAuditLog401 = {
    /**
     * @type boolean
     */
    success: false;
    error:
        | string
        | {
              [key: string]: any;
          };
};

/**
 * @description Server error.
 */
export type GetAuditLog500 = {
    /**
     * @type boolean
     */
    success: false;
    error:
        | string
        | {
              [key: string]: any;
          };
};

export type GetAuditLogQueryResponse = GetAuditLog200;

export type GetAuditLogQuery = {
    Response: GetAuditLog200;
    QueryParams: GetAuditLogQueryParams;
    Errors: GetAuditLog401 | GetAuditLog500;
};
